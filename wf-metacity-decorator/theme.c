 ...
 */
#define GETTEXT_PACKAGE "metacity_decor"
#define MARCO_DATADIR "/usr/share"
#define MARCO_LOCALEDIR "/usr/share/local"
//#include <config.h>
...
#else
  surface = cairo_image_surface_create(CAIRO_FORMAT_ARGB32, pixbuf_width, pixbuf_height);
...
meta_frame_layout_new  (void)
{
  ...
  layout->invisible_border.left = BORDERS_DELTA;
  layout->invisible_border.right = BORDERS_DELTA;
  layout->invisible_border.bottom = BORDERS_DELTA;
  layout->invisible_border.top = BORDERS_DELTA;
...
meta_initialize_button_layout (MetaButtonLayout *layout)
{
    ...
    layout->left_buttons[0] = META_BUTTON_FUNCTION_MENU; 
    layout->right_buttons[0] = META_BUTTON_FUNCTION_MINIMIZE;
    layout->right_buttons[1] = META_BUTTON_FUNCTION_MAXIMIZE;
    layout->right_buttons[2] = META_BUTTON_FUNCTION_CLOSE;
    layout->right_buttons[3] = META_BUTTON_FUNCTION_LAST;
}
...
const char * meta_button_function_to_string (MetaButtonFunction button)
{
    switch (button)
    {
        case META_BUTTON_FUNCTION_MENU: 
...
void
meta_update_button_layout (const char *value, MetaButtonLayout *new_layout)
{
	...
	while (buttons[b] != NULL)
	{
	    ...
	    else
	    {
		  ...
		  f = meta_button_opposite_function (f);
	       ...
	       {
		  fprintf (stderr, "Ignoring unknown or already-used "
			   "button name \"%s\"\n", buttons[b]);
	       }
	    }
	    b++;
	...
	if (sides[1] != NULL)
	{
	    ...
	    while (buttons[b] != NULL)
	    {
		  ...
		  if (f != META_BUTTON_FUNCTION_LAST)
		   ...
		   {
		       fprintf (stderr, "Ignoring unknown or "
...
gboolean
meta_get_button_position (gint    button_id,
                          MetaFrameGeometry *fgeom,
			              gint    *x,
			              gint    *y,
			              gint    *w,
...
void
meta_theme_draw_frame_test (MetaTheme         *theme,
                       GtkStyleContext        *style_gtk,
                       int                     client_width,
                       int                     client_height,
                       int                     text_height,
                       ...
                       )
{
  ...
  MetaFrameType type = META_FRAME_TYPE_NORMAL;
  MetaFrameFlags flags = common_flags;
                       ...
                       )
{
  ...
  {
    flags |= META_FRAME_FOCUS_YES;
    flags |= META_FRAME_IS_FLASHING;
  }
  if(state & STATE_MAXIMIZED)
  {
    flags |= META_FRAME_MAXIMIZED;
    flags &= ~(META_FRAME_ALLOWS_HORIZONTAL_RESIZE | META_FRAME_ALLOWS_VERTICAL_RESIZE);
  }
  else
  {
    flags &= ~META_FRAME_MAXIMIZED;
